...
依赖：reportlab（纯 Python）。中文字体：微软雅黑（msyh.ttc / msyhbd.ttc）。
...
from reportlab.platypus import (
    ...
    NextPageTemplate, Table, TableStyle, Image, KeepTogether,
)
...
# ── 样式 ──────────────────────────────────────────────────────
def _s(name, **kw):
    base = dict(fontName="YaHei", fontSize=10.5, leading=17, textColor=DARK)
    base.update(kw)
    return ParagraphStyle(name, **base)

ST = {
    "H1": _s("H1", fontName="YaHeiBd", fontSize=17, leading=22,
             textColor=DARK, spaceBefore=18, spaceAfter=8, keepWithNext=1),
    "H2": _s("H2", fontName="YaHeiBd", fontSize=12.5, leading=18,
             textColor=ACCENT_DARK, spaceBefore=12, spaceAfter=5, keepWithNext=1),
    "Body": _s("Body", alignment=TA_JUSTIFY, spaceAfter=6),
    "BodyC": _s("BodyC", alignment=TA_CENTER),
    "Li": _s("Li", leftIndent=14, bulletIndent=4, spaceAfter=3),
...
class ManualDoc(BaseDocTemplate):
    def __init__(self, filename, **kw):
        ...
        self.toc.levelStyles = TOC_LEVEL_STYLES
...
def cover_page(canvas, doc):
    ...
    if LOGO.exists():
        iw = 300
        ih = iw * 199 / 710
        canvas.drawImage(str(LOGO), (PAGE_W - iw) / 2, PAGE_H - 160 - ih,
                         width=iw, height=ih, mask="auto")

    # 主标题
    canvas.setFillColor(DARK)
    canvas.setFont("YaHeiBd", 30)
    canvas.drawCentredString(PAGE_W / 2, PAGE_H - 230, "GarmentStitcher")

    # 副标题
    canvas.setFont("YaHei", 15)
    canvas.setFillColor(GRAY)
    canvas.drawCentredString(PAGE_W / 2, PAGE_H - 266, "服装正反面拼接工具 · 使用说明书")

    # 强调色线
    canvas.setStrokeColor(ACCENT)
    canvas.setLineWidth(2.5)
    canvas.line(PAGE_W / 2 - 60, PAGE_H - 288, PAGE_W / 2 + 60, PAGE_H - 288)

    # 版本徽标
    canvas.setFillColor(ACCENT)
    canvas.roundRect(PAGE_W / 2 - 44, PAGE_H - 330, 88, 24, 12, fill=1, stroke=0)
    canvas.setFillColor(white)
    canvas.setFont("YaHeiBd", 12)
    canvas.drawCentredString(PAGE_W / 2, PAGE_H - 322, "版本 v2.0")
...
def bullets(items):
    out = []
    for it in items:
        out.append(Paragraph(it, ST["Li"], bulletText="•"))
    return out

def steps(items):
    out = []
    for i, it in enumerate(items, 1):
        out.append(Paragraph(it, ST["Li"], bulletText=f"{i}."))
    return out
...
def table(header, rows, widths=None):
    ...
    n = len(header)
    if widths is None:
        widths = [PAGE_W - 96 - 0] * n  # placeholder, overridden below
