...
| 功能 | 说明 |
|------|------|
| **AI+CV 联合检测** | rembg / 微调模型前景分割 → 逐行宽度分布 → 正反面共识区间 → 杆子底部裁剪 |
| **自动角度修正** | Theil-Sen 中轴 / 模板对齐 双算法可切换 + 手动拖拽旋转（框外近角点拖拽） |
| **流式处理** | 逐对处理，第一对完成即可审核，模型预热中点击 AI 处理会在就绪后自动开始 |
| **审核编辑** | BBox 拖拽（4 角 + 4 边中点 + 整体平移）+ 角度微调按钮 + 框外拖拽旋转 + 互换正反面 |
| **黄金比例拼接** | 正反面裁切位置按 φ 比例分配，外侧空隙大内侧空隙小，自然往中间靠拢 |
...
1. **浏览** 选择原图文件夹 → 弹出文件对话框直接看到图片内容，选任意一张即确定文件夹 → 模型后台预热
2. 点击 **AI 处理**（模型预热中点击会自动排队，就绪后自动开始。如有完整 `annotations.json` 则秒加载）
3. **左窗** 实时预览拼接结果（滚轮缩放）
...
③ 联合轮廓共识
   ...
   ↓
④ 共识提炼
   在最长共识区间内重新计算 bbox（水平和底部优化，顶部保留原值——微调模型已处理人台脖子）
   ↓
⑤ 杆子底部裁剪
   mask 宽度占比骤降 → 切除杆子/腿底部
   ↓
⑥ 自动角度修正
   检测服装中轴倾斜并补偿（见下方详解）
   ↓
⑦ 黄金比例裁切
   外侧空隙（近画幅边缘）∶ 中间总间隙（正+反之间）= φ ∶ 1
   ↓
⑧ 1:1 正方形拼接输出
```
...
| 轮次 | 数据量 | 最佳 loss | 说明 |
|------|--------|-----------|------|
| ~~Round 1-3~~ | ~~289 对~~ | — | ~~旧标注数据，已清空重做~~ |
| **最新** | **235 对** | **0.033** | 从 HF 预训练权重 + 全部新标注数据（上衣/连衣裙不擦人台，下衣擦上半身），25 epoch |
